...
def profile(username):
    ...
    return render_template('profile.html', current_user=user,
                           tweets=tweets, current_time=current_time,
...
def who_to_watch_list(user):
    return User.query.filter(User.id != user.id).order_by(
...
def timeline(username):
    ...
    if username:
        ...
        user = current_user
        tweets = Tweet.query.join(followers, (
            followers.c.followee_id == Tweet.user_id)).filter(
    ...
    return render_template('timeline.html', form=form,
                           tweets=tweets, current_time=current_time,
                           current_user=user, total_tweets=total_tweets,
                           who_to_watch=who_to_watch,
